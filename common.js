...
export async function loadHeaderAndFooter(currentPageId) {
    ...
    try {
        ...
        showModal("Yükleme Hatası", `Menü ve alt bilgi dosyaları yüklenemedi. Detay: ${error.message}`, 'error');
...
function updateNavAndAuthUI(currentPageId) {
    ...
    desktopLinks.forEach(link => {
        link.classList.remove('text-emerald-600', 'border-emerald-600', 'font-semibold'); 
    ...
    mobileLinks.forEach(link => {
        link.classList.remove('text-emerald-600', 'border-l-4'); 
        if (link.dataset.pageId === currentPageId) {
